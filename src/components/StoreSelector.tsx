  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-2">
        <input
          className="w-40 sm:w-56 md:w-64 px-2 sm:px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-xs sm:text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
          placeholder={loading ? "Carico..." : "Cerca negozio"}
          value={query}
      ...
      {query && (
        <div className="absolute mt-1 w-64 sm:w-72 max-h-64 overflow-auto bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md dark:shadow-gray-950/30 z-10">
          {filtered.length === 0 && <div className="px-3 py-2 text-xs sm:text-sm text-neutral-500 dark:text-gray-400">Nessun risultato</div>}
          {filtered.map(s => (
            <button key={s.id} onClick={()=>{ setSelectedId(s.id); setQuery(""); }} className="w-full text-left px-3 py-2 hover:bg-neutral-50 dark:hover:bg-gray-700 text-gray-900 dark:text-gray-100">
              ...
              <div className="text-xs text-neutral-500 dark:text-gray-400 truncate">{s.address || ""}</div>
            </button>
          ))}
        </div>
      )}
